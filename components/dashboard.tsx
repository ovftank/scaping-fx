    ...
    return (
        <>
            <div className='mb-6 flex justify-center sm:mb-8'>
                <button onClick={handleRefresh} className='flex w-full items-center justify-center gap-2 rounded-lg border border-white/20 bg-white/5 px-6 py-3 text-sm font-medium text-white transition-all hover:border-amber-500/50 hover:bg-amber-500/10 sm:w-auto sm:px-8'>
            ...
            {recommendation ? (
                <div className='relative overflow-hidden rounded-2xl border border-white/10 bg-[#111] shadow-2xl'>
                    ...
                    {/* Glow Effect */}
                    <div className={`pointer-events-none absolute -top-20 -right-20 h-64 w-64 rounded-full opacity-30 blur-[100px]`} style={{ backgroundColor: signalInfo.shadowColor }} />
                    ...
                    <div className='relative p-4 sm:p-8 sm:py-10'>
                        ...
                        <div className='mb-6 flex flex-col items-center gap-4 sm:mb-10 sm:gap-6'>
                            ...
                            <div className={`relative flex h-20 w-20 items-center justify-center rounded-full border-2 ${signalInfo.borderColor} bg-black/40 shadow-[0_0_40px_${signalInfo.shadowColor}] sm:h-28 sm:w-28`}>
                                ...
                                <span className={`material-symbols-outlined text-4xl ${signalInfo.textColor} relative z-10 sm:text-6xl`}>{signalInfo.icon}</span>
                            </div>
                            ...
                            <div className='text-center'>
                                <div className='mb-2 flex items-center justify-center gap-2'>
                                    ...
                                    <span className='text-[10px] font-semibold tracking-[0.2em] text-gray-400 uppercase sm:text-xs'>Tín hiệu</span>
                                </div>
                                <div className={`text-4xl font-black tracking-tighter uppercase ${signalInfo.textColor} drop-shadow-[0_0_30px_${signalInfo.shadowColor}] sm:text-6xl`}>{signalInfo.label}</div>
                            </div>

                            {/* Confidence Badge */}
                            <div className={`rounded-lg px-4 py-2 text-xs font-bold tracking-wider uppercase shadow-lg sm:rounded-xl sm:px-6 sm:py-3 sm:text-sm ${recommendation.confidence === 'STRONG' ? 'bg-linear-to-r from-white to-white/80 text-black' : recommendation.confidence === 'MODERATE' ? 'bg-linear-to-r from-white/70 to-white/50 text-black' : 'bg-linear-to-r from-white/40 to-white/20 text-gray-800'}`}>{recommendation.confidence === 'STRONG' ? 'Độ tin cậy: CAO' : recommendation.confidence === 'MODERATE' ? 'Độ tin cậy: TRUNG BÌNH' : 'Độ tin cậy: THẤP'}</div>
                        </div>
                        ...
                        <div className='mb-6 rounded-xl border border-white/10 bg-black/40 p-4 backdrop-blur-sm sm:mb-10 sm:p-6'>
                            <div className='mb-3 flex items-center gap-2 sm:mb-4 sm:gap-3'>
                                ...
                                <span className='text-xs font-bold tracking-wider text-gray-400 uppercase sm:text-sm'>Phân tích</span>
                            </div>
                            <p className='text-base leading-relaxed text-white sm:text-lg'>{recommendation.reasoning}</p>
                        </div>

                        {/* Price Stats - Modern Grid */}
                        <div className='mb-6 grid gap-3 sm:mb-10 sm:grid-cols-2 sm:gap-4'>
                            {/* Current Price */}
                            <div className='group relative overflow-hidden rounded-xl border border-white/10 bg-black/40 p-4 transition-all hover:border-white/20 sm:p-6'>
                                <div className='mb-2 flex items-center gap-2 sm:mb-3'>
                                    ...
                                    <span className='text-[10px] font-bold tracking-wider text-gray-500 uppercase sm:text-xs'>Giá hiện tại</span>
                                </div>
                            ...
                            <div className='group relative overflow-hidden rounded-xl border border-white/10 bg-black/40 p-4 transition-all hover:border-white/20 sm:p-6'>
                                <div className='mb-2 flex items-center gap-2 sm:mb-3'>
                                    <div className={`flex h-7 w-7 items-center justify-center rounded-lg ${recommendation.changeBuy >= 0 ? 'bg-green-500/10' : 'bg-red-500/10'} sm:h-8 sm:w-8`}>
                                        <span className={`material-symbols-outlined text-sm ${recommendation.changeBuy >= 0 ? 'text-green-500' : 'text-red-500'}`}>{recommendation.changeBuy >= 0 ? 'trending_up' : 'trending_down'}</span>
                                    </div>
                                    <span className='text-[10px] font-bold tracking-wider text-gray-500 uppercase sm:text-xs'>Thay đổi</span>
                                </div>
                                <div className='flex flex-col'>
                                    <span className={`font-mono text-2xl font-bold ${recommendation.changeBuy >= 0 ? 'text-green-500' : 'text-red-500'} sm:text-3xl`}>
                                        {recommendation.changeBuy >= 0 ? '+' : ''}
                                        {recommendation.changeBuy.toFixed(2)}
                                    </span>
                        ...
                        <div className='flex items-center justify-center gap-2 border-t border-white/10 pt-4 sm:gap-3 sm:pt-6'>
                            ...
                            <span className='text-xs text-gray-500 sm:text-sm'>Cập nhật: {new Date(recommendation.generatedAt).toLocaleString('vi-VN')}</span>
                        </div>
                    </div>
                </div>
            ) : (
